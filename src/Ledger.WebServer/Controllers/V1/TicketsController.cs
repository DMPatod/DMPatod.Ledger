...
namespace Ledger.WebServer.Controllers.V1
{
    ...
    [ApiController]
    public class TicketsController(IMessageHandler messageHandler, IMapper mapper) : ControllerBase
    {
        [HttpPost]
        ...
        public async Task<IActionResult> Get()
        {
            ...
            var result = await messageHandler.SendAsync(query, CancellationToken.None);
            if (result.IsFailed)
            {
                throw new Exception();
            }
            var response = mapper.Map<IEnumerable<TicketResponse>>(result.ValueOrDefault);
            return Ok(response);
        }
    }
}
